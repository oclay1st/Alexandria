...
from .schema import Model, DefaultHTTPErrorSchema
...
class Namespace(OriginalNamespace):
    ...
    def parameters(self, parameters, locations=None, as_kwargs=False):
        """
        ...
        def decorator(func):
            ...
            return self.doc(params=parameters)(
                self.response(code=HTTPStatus.UNPROCESSABLE_ENTITY)(
                    self.WEBARGS_PARSER.use_args(parameters, locations=_locations, as_kwargs=as_kwargs)(
                        func
                    )
                )
            )
